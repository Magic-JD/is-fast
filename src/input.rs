    ...
    if let event::Event::Key(KeyEvent { code, modifiers, .. }) = event::read()? {
        match code {
            ...
            KeyCode::Char('n') | KeyCode::Right if *index < links.len() - 1 => {
                ...
                *index += 1;
                *page = links.get(*index).map(|link| link.get_content()).unwrap_or_else(|| Paragraph::new(Text::from(String::from("Index out of bounds"))));
                *scroll_offset = 0;
...
fn draw(index: &mut usize, links: &[Link], page: &mut Paragraph, terminal: &mut Terminal<CrosstermBackend<Stdout>>, scroll_offset: &mut u16) -> Result<(), MyError> {
